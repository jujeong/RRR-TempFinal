...
import static com.example.subwayjpgsqluse.ui.AlarmLandingPageActivity.launchIntent;
...
public final class AlarmReceiver extends BroadcastReceiver {
    ...
    private static final long UPDATE_INTERVAL = TimeUnit.MINUTES.toMillis(1) * 10;
    private static final long NOTIFICATION_DURATION = TimeUnit.HOURS.toMillis(1);
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        try {
            arrivalData = new ArrivalReceiver().execute(alarm.getStationSelect()).get();
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        builder.setColor(ContextCompat.getColor(context, R.color.green_200));
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    String arrivalData = new ArrivalReceiver().execute(alarm.getStationSelect()).get();
                    bigTextStyle.bigText(arrivalData);
                    builder.setStyle(bigTextStyle);
                    manager.notify(id, builder.build());
    ...
    private static PendingIntent launchAlarmLandingPage(Context ctx, Alarm alarm) {
        return PendingIntent.getActivity(
                ctx, alarm.notificationId(), launchIntent(ctx), FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE
        );
    }
